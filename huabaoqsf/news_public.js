...
var tmp_arr=Domain.match(/[\w][\w-]*\.(?:com\.cn|net\.cn|org\.cn|com|cn|co|net|org|gov|cc|biz|info|hn|so|hk|us|us|mobi|wang|me|so|top)(\/|$)/);
Domain=tmp_arr[0];

var tmp_url = location.href;
if(tmp_url.indexOf("baike_add")==-1 && tmp_url.indexOf("bbs_add_book")==-1 && tmp_url.indexOf("bbs_thread")==-1 && tmp_url.indexOf("baike_edit")==-1 && tmp_url.indexOf("bbs_edit_book")==-1 && tmp_url.indexOf("/baike_")==-1 && tmp_url.indexOf("/thread_")==-1 && tmp_url.indexOf("/dom/sc_")==-1 && tmp_url.indexOf("/forum_")==-1 && tmp_url.indexOf("http://www.huabaoqsf.com/dom/denglu.php")==-1 && tmp_url.indexOf("http://www.huabaoqsf.com/dom/form_iframe.php")==-1 && tmp_url.indexOf("/zd_")==-1 && tmp_url.indexOf("/dom/zhidao")==-1){
...
$(function() {
    ...
    wsf.userLike = function(recordId, type, objectId, style, operate, user_name, part, html) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: 'UserOperate.php.htm'/*tpa=http://www.huabaoqsf.com/Public/UserOperate.php*/,
            data: {
                username: user_name,
                rid: recordId,
                opt: operate,
...
function copyToClipBoard(){
    ...
    if (window.clipboardData){
		...
		var divinfo = '<embed src="_clipboard.swf"/*tpa=http://www.huabaoqsf.com/_clipboard.swf*/ FlashVars="clipboard='+clipBoardContent+'" width="0" height="0" type="application/x-shockwave-flash"></embed>';
...
function set_msg_new(width,height,title,content,true_fn,false_fn,no_close){
	...
	var float_div = '<div class="msg_main"  id="float" style="left:'+left_width+'px;top:'+top_height+'px;width:'+width+'px;height:'+height+'px;"><div class="msg_tit"><strong>'+title+'</strong><span class="msg_close" '+close_style+'><a href="javascript:remove_div(\'float\')"><img src="18-1.gif"/*tpa=http://img.ev123.com/pic/vip_msg/18.gif*/ border="0" /></a></span></div><div class="msg_contents"><span class="msg_info">'+content+'</span></div><div class="msg_buttons" '+ css_style +'><input type="button"  class="msg_button" value="确 定" onclick="'+true_fn+'" />'+ false_html +'</div><div class="msg_foot"><a href="#"></a></div></div>';
		...
		$.post(
            ...
            function (data) {
                ...
                if (userPro && userPro.length != 0) {
                    ...
                    var curRebateInfo = '&yen;'+cur_rebate['rebatePrice']+'<font>'+cur_rebate['level_name']+'专享 '+cur_rebate['rebateNum']+'折</font>';
...
$(function(){
  ...
  if(!zz_userid || !user_name){
		...
		$.post("ajax_get_user_info.php.htm"/*tpa=http://www.huabaoqsf.com/ajax_get_user_info.php*/, {type:"1",u:time,username:user_name,zz_userid:zz_userid},
			function(data){
				$(".top-login").html(data);
			}
		);
